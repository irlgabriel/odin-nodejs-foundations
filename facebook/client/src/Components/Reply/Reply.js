...
const Reply = ({level = 0, reply, replies, setReplies, comment, user, post}) => {
  ...
  const deleteHandler = () => {
    ...
    axios.delete(`/posts/${post._id}/comments/${reply._id}`, config)
    .then(res => {
  ...
  const likeComment = () => {
    axios.post(`/posts/${post._id}/comments/${reply._id}`, {}, config)
    .then(res => {
      setReplies(replies.map(reply => reply._id === res.data._id ? res.data : reply));
    })
    .catch(err => console.log(err));
  }
  ...
  const editHandler = (e) => {
    e.preventDefault();
    axios.put(`/posts/${post._id}/comments/${reply._id}`, {content}, config)
    .then(res => {
      setReplies(replies.map(reply => reply._id === res.data._id ? res.data : reply))
      setEdit(false);
    })
  ...
  return (
    <ReplyContainer>
      ...
      <ReplyWrapper className={'w-100'}>
        <ReplyBody>
        <h6 className='mb-0'>{reply.user.display_name || reply.user.first_name + ' ' + reply.user.last_name}</h6>
        {
          !showEdit 
          ? <p className='mb-0' dangerouslySetInnerHTML={{__html: reply.content}} ></p>
          : 
        ...
        }
        {
          !showEdit && 
          <LikesContainer>
            <AiFillLike fill={reply.likes.some(e => e._id === user._id) ? 'royalblue' : ''} size={12}/>
            &nbsp;
            <p style={{fontSize: '12px'}} className='d-inline-block mb-0'>{reply.likes.length}</p>
          </LikesContainer>
        }
        </ReplyBody>
        <ReplyFooter>
          <FooterLink color={reply.likes.some(e => e._id === user._id) ? 'royalblue' : 'black'} onClick={() => likeComment()} bold>
            Like
          </FooterLink>
          {
